...
{ config, lib, pkgs, ... }:
let
  eiros_pay_respects = config.eiros.system.default_applications.pay_respects;
  eiros_zsh = config.eiros.system.default_applications.zsh;
in
{
  options.eiros.system.default_applications.pay_respects = {
    enable = lib.mkOption {
      default = true;
      description = "Install pay-respects to correct mistyped shell commands (type the alias after a failed command to fix it).";
      example = lib.literalExpression ''
        {
          eiros.system.default_applications.pay_respects.enable = false;
        }
      '';
      type = lib.types.bool;
    };
    ...
    alias = lib.mkOption {
      ...
      example = lib.literalExpression ''
        {
          eiros.system.default_applications.pay_respects.alias = "fuck";
        }
      '';
      type = lib.types.str;
    };
