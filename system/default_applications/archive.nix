...
{ config, lib, pkgs, ... }:
let
  eiros_archive = config.eiros.system.default_applications.archive;
in
{
  options.eiros.system.default_applications.archive = {
    enable = lib.mkOption {
      default = true;
      description = "Install archive creation tools.";
      example = lib.literalExpression ''
        {
          eiros.system.default_applications.archive.enable = false;
        }
      '';
      type = lib.types.bool;
    };
    ...
    zip.enable = lib.mkOption {
      ...
      example = lib.literalExpression ''
        {
          eiros.system.default_applications.archive.zip.enable = false;
        }
      '';
      type = lib.types.bool;
    };
    ...
    p7zip.enable = lib.mkOption {
      ...
      example = lib.literalExpression ''
        {
          eiros.system.default_applications.archive.p7zip.enable = false;
        }
      '';
      type = lib.types.bool;
    };
