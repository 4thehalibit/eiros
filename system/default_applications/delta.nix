...
{ config, lib, pkgs, ... }:
let
  eiros_delta = config.eiros.system.default_applications.delta;
  eiros_git = config.eiros.system.default_applications.git;
in
{
  options.eiros.system.default_applications.delta = {
    enable = lib.mkOption {
      default = true;
      description = "Install delta and configure it as the git diff pager.";
      example = lib.literalExpression ''
        {
          eiros.system.default_applications.delta.enable = false;
        }
      '';
      type = lib.types.bool;
    };
    ...
    side_by_side.enable = lib.mkOption {
      ...
      example = lib.literalExpression ''
        {
          eiros.system.default_applications.delta.side_by_side.enable = false;
        }
      '';
      type = lib.types.bool;
    };
    ...
    navigate.enable = lib.mkOption {
      ...
      example = lib.literalExpression ''
        {
          eiros.system.default_applications.delta.navigate.enable = false;
        }
      '';
      type = lib.types.bool;
    };
    ...
    line_numbers.enable = lib.mkOption {
      ...
      example = lib.literalExpression ''
        {
          eiros.system.default_applications.delta.line_numbers.enable = false;
        }
      '';
      type = lib.types.bool;
    };
