...
{ config, lib, pkgs, ... }:
let
  eiros_multimedia = config.eiros.system.default_applications.multimedia;
in
{
  options.eiros.system.default_applications.multimedia = {
    enable = lib.mkOption {
      default = true;
      ...
      example = lib.literalExpression ''
        {
          eiros.system.default_applications.multimedia.enable = false;
        }
      '';
      type = lib.types.bool;
    };
    ...
    packages = lib.mkOption {
      ...
      example = lib.literalExpression ''
        {
          eiros.system.default_applications.multimedia.packages = with pkgs.gst_all_1; [
            gstreamer
            gst-plugins-base
            gst-plugins-good
          ];
